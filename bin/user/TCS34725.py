...
class TCS34725:
    ...
    def __init__(self, address=0x29, debug=False):
        ...
        while RestartCount < 3:
            try:
                ...
                print(" Exception during GPIO Init "+ str(Exception))
    ...
    def ChangeGain(self,dir):
        if (self.Gain_t== self.TCS34725_GAIN_1X):
            if (dir==1):
                self.Set_Gain(self.TCS34725_GAIN_4X)
                self.Gain_t = self.TCS34725_GAIN_4X
                
            
        elif (self.Gain_t== self.TCS34725_GAIN_4X):
            if (dir==1):
                self.Set_Gain(self.TCS34725_GAIN_16X)
                self.Gain_t = self.TCS34725_GAIN_16X
